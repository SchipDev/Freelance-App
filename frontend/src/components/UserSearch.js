...
class UserSearch extends Component {
    ...
    handleSubmitJob = e => {
        ...
        actions.searchByJobTitle(queStr).then(res => this.setState({ results: res.data, lastSearch: this.state.job }))
    }

    displayResults = () => {
        if (this.state.results.length === 0) { return <div>No Results For ${this.state.lastSearch}</div> }
    ...
    render() {
        return (
            <div>
                ...
                <div>
                    {this.state.results != 'nsy' ? this.displayResults() : 'Search for people by their job title or name. '}
                </div>
            </div>
        )
    }
