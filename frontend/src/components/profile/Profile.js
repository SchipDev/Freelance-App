...
class Profile extends Component {
  ...
  deleteWE = ind => {
    actions.deleteWE(this.state.userResume?._id, {remove: ind}).then(res => this.setState({userResume: res.data, isEditingWE: false}))
  }

  deleteSkill = ind => {
    actions.deleteSkill(this.state.userResume?._id, {remove: ind}).then(res => this.setState({userResume: res.data, isEditingSkills: false}))
  }

  deleteEdu = ind => {
    actions.deleteEdu(this.state.userResume?._id, {remove: ind}).then(res => this.setState({userResume: res.data, isEditingEdu: false}))
  }
  ...
  displayWorkExperience = () => {
    let workExp = this.state.userResume?.workExperience?.map((we, ind) => {
      return (
        <div className="workExp2">
          ...
          {this.state.isEditingWE ? <button onClick={() => this.deleteWE(ind)}>Remove</button> : ''}
        </div>
      );
    });
    ...
    return (
      <div className="workExp1">
        <h3 className="headRes">Work Experience <button onClick={() => this.setState({isEditingWE: !this.state.isEditingWE})}>Edit</button></h3>
        <div>{workExp}</div>
        {this.state.userResume?.workExperience?.length < 4 ? <p
          onClick={() =>
  ...
  displaySkills = () => {
    let skillList = this.state.userResume?.skills?.map((skill, ind) => {
      return <li className="skillSet">{skill} {this.state.isEditingSkills ? <button onClick={() => this.deleteSkill(ind)}>Remove</button> : ''}</li>;
    });

    return (
      <div className="skills1">
        <h3 className="headRes">Skills <button onClick={() => this.setState({isEditingSkills: !this.state.isEditingSkills})}>Edit</button></h3>
        <ul>{skillList}</ul>
        {this.state.userResume?.skills?.length < 10 ? <p
          onClick={() =>
  ...
  displayEdu = () => {
    let eduList = this.state.userResume?.education?.map((edu, ind) => {
      return (
        <div className="workExp2">
          ...
          {this.state.isEditingEdu ? <button onClick={() => this.deleteEdu(ind)}>Remove</button> : ''}
        </div>
      );
    });
    ...
    return (
      <div className="edu1">
        <h3 className="headRes">Education <button onClick={() => this.setState({isEditingEdu: !this.state.isEditingEdu})}>Edit</button></h3>
        <div>{eduList}</div>
        {this.state.userResume?.education?.length < 3 ? <p
          onClick={() =>
